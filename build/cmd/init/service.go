	...
	Container("dns",
		...
		dns.Service(
			...
			dns.Zone("app.test", "ns1.app.test", "wojtek@app.test", 1,
				...
				dns.Domain("ns1.app.test", "10.255.0.2"),
				dns.Domain("smtp.app.test", dev.SMTPAddr),
				dns.Text("_dmarc.app.test", "v=DMARC1;p=quarantine"),
				dns.Text("app.test", "v=spf1 a:mailer.app.test ~all"),
				dns.Domain("app.test", "10.255.0.2"),
				dns.Domain("mailer.app.test", "10.255.0.2"),
			),
		),
	),
	Container("acme",
