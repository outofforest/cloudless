...
var devContainer = BoxFactory(
	...
	containercache.Mirrors("http://10.255.0.6:81"),
)

var HostDev = Join(
	Host("dev",
		MountPersistentBase("vda"),
		Network("fc:ff:ff:ff:00:01", "igw", IPs("10.101.0.4/24")),
		Gateway("10.101.0.1"),
		shield.Expose("tcp", "10.101.0.4", ingress.PortHTTP, "10.255.0.5", ingress.PortHTTP),
		shield.Expose("tcp", "10.101.0.4", pebble.Port, "10.255.0.2", pebble.Port),
		shield.Expose("tcp", "10.101.0.4", smtp.SMTPPort, "10.255.0.3", smtp.SMTPPort),
		shield.Expose("tcp", "10.101.0.4", containercache.Port, "10.255.0.6", containercache.Port),
		shield.Expose("udp", "10.101.0.4", dns.Port, "10.255.0.7", dns.Port),
		shield.Masquerade("brint", "igw"),
		Bridge("brint", "fc:ff:ff:ff:01:01", IPs("10.255.0.1/24")),
	...
	devContainer("pebble",
		Network("fc:ff:ff:ff:01:02", "igw", IPs("10.255.0.2/24")),
		Gateway("10.255.0.1"),
		shield.Open("tcp4", "igw", pebble.Port),
		pebble.Container("pebble", "10.101.0.4:53"),
	),
	devContainer("smtp",
		Network("fc:ff:ff:ff:01:03", "igw", IPs("10.255.0.3/24")),
		Gateway("10.255.0.1"),
	...
	devContainer("webmail",
		Network("fc:ff:ff:ff:01:04", "igw", IPs("10.255.0.4/24")),
		Gateway("10.255.0.1"),
		shield.Open("tcp4", "igw", webmail.Port),
		webmail.Container("10.255.0.3:25", "10.255.0.3:143"),
	),
	devContainer("ingress",
		Network("fc:ff:ff:ff:01:05", "igw", IPs("10.255.0.5/24")),
		Gateway("10.255.0.1"),
		...
		ingress.Service(
			...
			ingress.Target("mail", "10.255.0.4", webmail.Port, "/"),
		),
	),
	devContainer("cache",
		Network("fc:ff:ff:ff:01:06", "igw", IPs("10.255.0.6/24")),
		Gateway("10.255.0.1"),
	...
	devContainer("dns",
		Network("fc:ff:ff:ff:01:07", "igw", IPs("10.255.0.7/24")),
		Gateway("10.255.0.1"),
