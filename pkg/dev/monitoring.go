...
var monContainer = BoxFactory(
	dns.DNS(),
	eye.SendMetrics("http://10.255.255.3"),
...
var monitoringBox = Join(
	Box("mon",
		...
		Gateway("10.255.0.1"),
		shield.Expose("tcp", "10.255.0.253", 81, "10.255.255.3", prometheus.Port),
	...
	monContainer("mon-grafana",
		...
		grafana.Container("grafana",
			grafana.DataSource("Prometheus", grafana.DataSourcePrometheus, "http://10.255.255.3:"+strconv.Itoa(prometheus.Port)),
