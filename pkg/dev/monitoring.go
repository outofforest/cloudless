...
var monitoringBox = Join(
	Box("mon",
		...
		Gateway("10.255.0.1"),
		shield.Expose("tcp", "10.255.0.253", 81, "10.0.0.3", prometheus.Port),
		shield.Expose("tcp", "10.255.0.253", 82, "10.0.0.4", loki.Port),
		shield.Expose("tcp", "10.255.0.253", ingress.PortHTTP, "10.0.0.5", ingress.PortHTTP),
		shield.Expose("udp", "10.255.0.253", dns.Port, "10.0.0.6", dns.Port),
		shield.Masquerade("brint", "igw"),
		Bridge("brint", "fc:ff:ff:fe:01:01", IPs("10.0.0.1/24")),
	...
	monContainer("mon-grafana",
		...
		grafana.Container("grafana",
			grafana.DataSource("Prometheus", grafana.DataSourcePrometheus, "http://10.0.0.3:"+strconv.Itoa(prometheus.Port)),
			grafana.DataSource("Loki", grafana.DataSourceLoki, "http://10.0.0.4:"+strconv.Itoa(loki.Port)),
